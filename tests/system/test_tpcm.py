...
def tpcm_hooks(request):
    ...
    if not utils.retry_api(basic.tpcm_operation,vars.D1, action="install",docker_name=tpcm_dict["httpd_docker"],
                           install_method="pull",image_path="httpd",tag_name="latest",
						   extra_args="--privileged -p 80:80 -v {} ".format(tpcm_dict["httpd_path"]),
                           retry_count=3, delay=10,skip_error=True):
        st.error("########## Docker {} pull from hub registry failed ##########".format(tpcm_dict["httpd_docker"]))
    st.wait(5)
...
def test_FtOpSoStTpcm_21_3(tpcm_install_fixture):
    ...
    for action in ["warm","config"]:
        ...
        if basic.verify_tpcm_list(vars.D1,docker_list=[tpcm_dict["tpc5_name"]],image_list=[tpcm_dict["httpd_image"]],
                                  status_list=["Up"]):
            st.log("########## PASS: {} status is \"Up\" as expected after {} reboot"
                   " #########".format(tpcm_dict["tpc5_name"], action))
        else:
            st.error("########## FAIL: status of {} is not \"Up\" after {} reboot"
                     " #########".format(tpcm_dict["tpc5_name"], action))
            success = False
    ...
    if basic.verify_tpcm_list(vars.D1, docker_list=[tpcm_dict["tpc1_name"], tpcm_dict["tpc2_name"],
        ...
        success = False
    if success:
        st.report_pass("test_case_id_passed", "test_FtOpSoStTpcm_21_3")
    else:
        st.report_fail("test_case_id_failed", "test_FtOpSoStTpcm_21_3")
