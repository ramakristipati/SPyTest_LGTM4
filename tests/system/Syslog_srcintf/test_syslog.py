...
def test_rsyslog_default_vrf(fixture_test_rsyslog_default_vrf):
    ...
    st.log('Get number of lines in the log file')
    total_lines = basic_obj.get_number_of_lines_in_file(data.dut2_server, file_path=data.remote_syslog_path, device='sonic_device')
    ...
    st.log('Check if execution of command - show ip route logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'show ip route', start_line=total_lines+1)
    if result is False:
    ...
    st.log('Check if messages from hostcfgd are logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'hostcfgd', start_line=total_lines+1)
    if result is False:
    ...
    st.log('Check if show vlan is logged')
    loc_lib.syslog_debug()
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'show Vlan '+data.dut1_dut2_vlan[0], start_line=total_lines+1)
    if result is False:
    ...
    st.log('Check if message addLagMemberToHardware is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'addLagMemberToHardware', start_line=total_lines+1)
    if result is False:
...
def test_rsyslog_negative():
    ...
    st.log('Check if message D1 is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'D1', start_line=total_lines+1)
    if result is True:
...
def test_rsyslog_nondefault_vrf():
    ...
    st.log('Check if message updatePortOperStatus is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'updatePortOperStatus', start_line=total_lines+1)
    if result is False:
    ...
    st.log('Check if message hostcfgd is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'hostcfgd', start_line=total_lines+1)
    if result is False:
    ...
    st.log('Check if message show vlan is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'show Vlan '+data.dut2_dut3_vlan[0], start_line=total_lines+1)
    if result is False:
        error = "Syslogs verification on a non-default VRF failed"
        tc_result = False ; error_list.append(error)
    ...
    st.log('Check if message addRoute is logged')
    result = retry_api(basic_obj.check_error_log, data.dut2_server, data.remote_syslog_path, 'addRoute', start_line=total_lines+1)
    if result is False:
        error = "Syslogs verification on a non-default VRF failed"
        tc_result = False ; error_list.append(error)
