...
def debug_traffic_fail():
    ...
    utils.exec_all(False, [[intf.show_interface_counters_all, dut] for dut in dut_list])
...
def test_mclag_fallback_ICL_shut(enable_client_ports):
    ...
    ### verify PO1 state is up
    retry_func(verify_po_state,po_name_list=['PortChannel1'], state='up',retry_count=3,delay=2)
...
def test_mclag_fallback_active_standby_coldboot(enable_client_ports):
    ...
    ### verify PO1 state is up
    print_log("Verify Keep-alive link is UP",'MED')
    retry_func(verify_po_state, po_name_list=['PortChannel1'], state='up', retry_count=3, delay=2)
