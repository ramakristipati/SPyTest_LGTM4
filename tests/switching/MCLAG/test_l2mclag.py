...
def test_flap_orphanPorts_clientPorts():
    ...
    if retry_func(verify_mclag_intf_state,mclag_intf_data=mclag_intf_data,retry_count=3,delay=2):
        print_log("MCLAG Interfaces State verification PASSED", "HIGH")
    else:
        print_log("MCLAG Interfaces State verification FAILED", "HIGH")
        mclag_intf_fail += 1
...
def test_peer_connection_down(lag_function_fixture):
    ...
    if retry_func(verify_mclag_state, mclag_data=mclag_data, retry_count=3,delay=2):
...
@pytest.fixture(scope="function")
def config_deconfig_bum_traffic():
    ### Increase mac dampening threshold as suggested in 32357 to avoid mac move dampening with BUM traffic
    st.banner("Configure Higher Mac Move Dampening threshold 50 so that ports not disabled")
    utils.exec_foreach(True, dut_list, mac.configure_macmove_threshold, count=50, interval=5)

    yield
    start_stop_traffic(src_stream_list=bum_src_streams, direction='single', action_ctrl='STOP')
    ### Configure back default mac dampenign threshold values
    utils.exec_foreach(True, dut_list, mac.configure_macmove_threshold, count=5, interval=5)
    #utils.exec_all(pll_exec, [[mac_obj.clear_mac_dampening, dut] for dut in data.my_dut_list])


def test_l2mclag_bum_traffic(lag_function_fixture,config_deconfig_bum_traffic):
    '''
        Verify MCLAG bring up with PortChannel Interfaces
    '''
    pre_result_handler()
...
def test_l2mclag_docker_restart():
    ...
    result = retry_parallel(basic.get_system_status, dict_list=[None,None], dut_list=mclag_peers, retry_count=20, delay=5)
    if not result:
