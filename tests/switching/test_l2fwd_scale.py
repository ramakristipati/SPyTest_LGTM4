...
def initialize_topology():
    ...
    data.dut_mac_count_list = [2 * dut_base_mac_count for _ in range(len(data.ver_dut_list))]
...
def verify_macs_ageout(dut_list,mac_list,aging_time,poll_interval,max_wait,**kwargs):
    ...
    for _ in range(poll_interval,max_wait,poll_interval):
...
def test_stp_mac_perf(cleanup_stp_mac_move):
    ...
    expect_mac_list = [expected_mac for _ in [d1,d2]]
    ...
    expected_mac = 2 * data.stp_macs + data.static_mac_count
    expect_mac_list = [expected_mac for _ in data.my_dut_list]
...
def test_scale_vlans_macs():
    ...
    print_log("snmp testing with max mac address scaling scenario")
    snmp_max_mac_scale()
...
def snmp_max_mac_scale():
    """
    Author : Prasad Darnasi<prasad.darnasi@broadcom.com>
    Verify The Q-BRIDGE-MIB Scalar requirements functionality by learning max mac addressess on DUT
    """
    ...
    snmp_output1 = snmp_obj.poll_for_snmp_walk(vars.D1, 30, 1, ipaddress=ipaddress,
                                                oid=data.oid_sysName, community_name=data.ro_community)
    basic_obj.get_top_info(vars.D1, proc_name='snmpd')
    snmp_output2 = snmp_obj.poll_for_snmp_walk(vars.D1, 14, 5, ipaddress=ipaddress, oid=data.oid_dot1qFdbTable,
                                               community_name=data.ro_community)
    basic_obj.get_top_info(vars.D1, proc_name='snmpd')

    if snmp_output1 and snmp_output2:
        st.report_tc_pass("ft_snmp_scaling02", "test_case_passed")
    else:
        st.log('Generating the tech support for the subtest')
        basic_obj.get_techsupport(filename='snmp_max_mac_scale')
        st.report_tc_fail("ft_snmp_scaling02", "get_snmp_output_fail")
