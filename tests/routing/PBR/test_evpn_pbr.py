...
def test_pbr_vxlan_triggers(cleanup_fixture_03):
    ...
    for trigger,tc in zip(trigger_list,tc_list):
        if trigger == 'clear_bgp':
            ...
            st.wait(20, 'waiting for PBR counters to increment after Trigger clear bgp')
            ...
            st.wait(60, 'waiting for PBR counters to increment after Trigger fast reboot')
            ...
            st.wait(20, 'waiting for PBR counters to increment after Trigger reboot of the spine device')
            [res, exceptions] = st.exec_all([[leaf1],[leaf2], [leaf3], [leaf4]])
            ...
            st.wait(60, 'waiting for PBR counters to increment after Trigger warm reboot')
            [res, exceptions] = st.exec_all([[leaf1],[leaf2], [leaf3], [leaf4]])
            ...
            #st.reboot(data.dut4, 'warm')
            st.wait(60, 'waiting for PBR counters to increment after Trigger config reload')
            [res, exceptions] = st.exec_all([[leaf1],[leaf2], [leaf3], [leaf4]])
            ...
            st.wait(20, 'waiting for PBR counters to increment after Trigger bgp docker restart')
            [res, exceptions] = st.exec_all([[leaf1],[leaf2], [leaf3], [leaf4]])
