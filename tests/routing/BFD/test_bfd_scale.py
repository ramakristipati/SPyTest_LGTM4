...
def test_FtOpSoRoBfdSc085(prologue_epilogue):
    ...
    if test_type != 'mix':
        ...
        intf_multiplier = 2
    result = retry_api(bfd.verify_bfd_peer,dut1, peer=nbr_list,interface=vlan_intf*intf_multiplier,status=['up']*max_bfd,rx_interval=[['300','300']]*max_bfd,tx_interval=[['300','300']]*max_bfd,retry_count=5,delay=1,ping_verify=True)
    if result is False:
        err = "BFD parameters are incorrect for one or more BFD peers"
        err_list.append(err);tc_result =False
        st.error(err)
    result = retry_api(bfd.verify_bfd_peer,dut1, vrf_name=user_vrf_name, peer=nbr_list_vrf,interface=vlan_intf_vrf*intf_multiplier,status=['up']*max_bfd_vrf,rx_interval=[['300','300']]*max_bfd_vrf,tx_interval=[['300','300']]*max_bfd_vrf,retry_count=5,delay=1,ping_verify=True)
    if result is False:
        err = "BFD parameters are incorrect for one or more BFD peers"
        err_list.append(err);tc_result =False
        st.error(err)
