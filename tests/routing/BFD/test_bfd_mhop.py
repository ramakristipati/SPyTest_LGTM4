...
def verify_bfd_lag1(vrfname='default'):
    ...
    if vrfname == 'default':
        ...
        ###########################################################################################
        result = retry_api(bfd.verify_bfd_peer,dut1,peer=[bfd_nbrs_dut1[0],ipv6_bfd_nbrs_dut1[0]],interface=[intf_list[0],intf_list[0]],status=['up','up'],local_addr=["",ipv6_bfd_nbrs_dut3[0]],peer_type=["dynamic","dynamic"],retry_count=3,delay=1,ping_verify=True)
        if result is False:
            st.log("FAILED : BFD session parameters mismatch for %s and %s" %(bfd_nbrs_dut1[0],ipv6_bfd_nbrs_dut1[0]))
            ret_val = False

        if ret_val is False: basic.get_techsupport(filename='bfd_lag1_{}'.format(vrfname))
...
def verify_bfd_lag2(vrfname='default'):
    ...
    result = retry_api(bfd.verify_bfd_peer, dut1, vrf_name=vrfname, peer=[bfd_nbrs_dut1[0], non_bfd_nbrs_dut1[0]],
                       ...
                       peer_type=["configured", "configured"], ping_verify=True)
    if result is False:
...
def verify_bfd_lag4():
    ...
    result = retry_api(bfd.verify_bfd_peer,dut1,peer=[dut3_l3lagip_list[0],dut3_l3lagipv6_list[0]],interface=[lag_name3]*2,status=['up']*2,retry_count=2,delay=1,ping_verify=True)
    if result is False:
...
def verify_bfd_lag6():
    ...
    result = retry_api(bfd.verify_bfd_peer, dut1, vrf_name=user_vrf_name, peer=[dut3_lo_ip, dut3_lo_ipv6],
                       ...
                       retry_count=5, delay=2, peer_type=["dynamic", "dynamic"], ping_verify=True)
    if result is False:
        st.log("FAILED : BFD session parameters mismatch for %s and %s" % (dut3_lo_ip, dut3_lo_ipv6))
        ret_val = False
    basic.add_user_log_in_frr(dut1, "bfd.log")
    ...
    for nbr in [dut3_lo_ip, dut3_lo_ipv6]:
        for _ in range(5):
            result1 = ip_bgp.verify_bgp_neighbor(dut1, vrf=user_vrf_name, neighborip=nbr, state='Established')
            if not result1:
                break
            st.wait(1)
        result = retry_api(ip_bgp.verify_bgp_neighbor, dut1, vrf=user_vrf_name, neighborip=nbr,
                           bgpdownreason="BFD down received", retry_count=5, delay=1)
        if result is False and result1 is True:
            st.log("FAILED:BGP neighbor state is incorrect for %s" % nbr)
            ret_val = False
    ...
    if result is True:
        st.log("FAILED : BFD session parameters mismatch for %s and %s" % (dut3_lo_ip, dut3_lo_ipv6))
        ret_val = False
    if not ret_val: basic.get_techsupport(dut_list, 'test_bfd_lag_006_port_flap')
    ...
    if not ret_val: basic.get_techsupport(dut_list, 'test_bfd_lag_006_port_flap_physical')
...
def verify_bfd_lag7():
    ...
    for testreloads in ['config_reload', 'clear_bgp', None]:
        ...
        result = retry_api(bfd.verify_bfd_peer, dut1, vrf_name=user_vrf_name, peer=[dut3_lo_ip, dut3_lo_ipv6],
                           local_addr=[dut1_lo_ip, dut1_lo_ipv6], status=['up', 'up'], multihop=["yes", "yes"],
                           retry_count=10, delay=3, peer_type=["dynamic", "dynamic"], ping_verify=True)
        if result is False:
            st.log("FAILED : BFD session parameters mismatch for %s and %s" % (dut3_lo_ip, dut3_lo_ipv6))
            ret_val = False
...
def verify_bfd_lag11():
    ...
    result = retry_api(bfd.verify_bfd_peer, dut1, vrf_name=user_vrf_name, peer=d2_prt_link_local, local_addr=d1_prt_link_local,
                       interface=access_vlan_name, status='up', retry_count=2, delay=1, ping_verify=True)
    if result is False:
