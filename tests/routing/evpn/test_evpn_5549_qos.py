...
def test_FtOpSoRoEvpnQos321(l2_uniform_fixture):
    ...
    port = [vars.D1D3P1,vars.D1D3P2,vars.D1D3P3,vars.D1D3P4]
    if not utils.retry_api(verify_queuing,vars.D1, port_list=port, queue="UC5",retry_count=3, delay=10):
        success=False
    ...
    port = [vars.D3T1P1]
    if not utils.retry_api(verify_queuing,vars.D3, port_list=port, queue="UC5",
                           retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]):
        success=False
...
def test_FtOpSoRoEvpnQos326(l2_pipe_fixture):
    ...
    for dut,queue,port in zip([vars.D3],["UC3"],[[vars.D3T1P1]]):
        ...
        if not utils.retry_api(verify_queuing,dut, port_list=port, queue=queue,
                               retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]):
            success=False
...
def test_FtOpSoRoEvpnQos322(tc322_uniform_fixture):
    ...
    for header in ["IPv4", "IPv6"]:
        ...
        if not utils.retry_api(verify_queuing,vars.D1, port_list=[vars.D1D3P1, vars.D1D3P2, 
                                                                  vars.D1D3P3, vars.D1D3P4],
                               queue="UC3",retry_count=3, delay=10, val_list=["3500"],tol_list=["2500"]):
            success = False
        ...
        if not utils.retry_api(verify_queuing,vars.D1, port_list=[vars.D1D4P1, vars.D1D4P2, 
                                                                  vars.D1D4P3, vars.D1D4P4], 
                               queue="UC5",retry_count=3, delay=10,val_list=["3500"],tol_list=["2500"]):
            success = False
...
def test_FtOpSoRoEvpnQos327(tc327_pipe_fixture):
    ...
    for header in ["IPv4", "IPv6"]:
        ...
        if not utils.retry_api(verify_queuing,vars.D1, port_list=[vars.D1D3P1, vars.D1D3P2, 
                                                                  vars.D1D3P3, vars.D1D3P4], 
                               queue="UC2",retry_count=3, delay=10,val_list=["3500"],tol_list=["2500"]):
            success = False
        ...
        if not utils.retry_api(verify_queuing,vars.D1, port_list=[vars.D1D2P1, vars.D1D2P2, 
...
def test_FtOpSoRoEvpnQos324(arp_uniform_fixture):
    ...
    for proto in ["ARP","ND"]:
        ...
        if proto == "ND":
            ...
            st.wait(10)
            port=[vars.D1D2P1, vars.D1D2P2, vars.D1D2P3, vars.D1D2P4]
            if not utils.retry_api(verify_queuing,vars.D1, port_list=port, queue="UC2",retry_count=3, delay=10):
                success = False
            ...
            port=[vars.D2T1P1] 
            if not utils.retry_api(verify_queuing,vars.D2, port_list=port, queue="MC12",
            ...
            st.wait(10)
            port=[vars.D1D3P1, vars.D1D3P2, vars.D1D3P3, vars.D1D3P4]
            if not utils.retry_api(verify_queuing,vars.D1, port_list=port, queue="UC5",
                                                                            retry_count=3, delay=10):  
                success = False
            ...
            if not utils.retry_api(verify_queuing,vars.D3, port_list=[vars.D3T1P1], queue="MC15",
                                                                          retry_count=3, delay=10):
                success = False
...
def test_FtOpSoRoEvpnQos329(arp_pipe_fixture):
    ...
    for proto in ["ARP", "ND"]:
        ...
        if proto == "ND":
            ...
            tg.tg_packet_control(port_handle=tg_dict["d2_tg_ph1"], action='stop')
            port=[vars.D1D2P1, vars.D1D2P2, vars.D1D2P3, vars.D1D2P4]
            if not utils.retry_api(verify_queuing,vars.D1, port_list=port, queue="UC6",
                                   retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]):
                success = False
            ...
            port=[vars.D2T1P1]
            if not utils.retry_api(verify_queuing,vars.D2, port_list=port, queue="MC15",retry_count=3, delay=10):
                success = False
            ...
            if Mac.verify_mac_address_table(dut=evpn_dict["leaf_node_list"][1],
                ...
                st.log("########## FAIL: MAC {} NOT present in D3, ARP request from D2 to D3"
                       " is not sent ##########".format(evpn_dict["leaf1"]["tenant_mac_v4_colon"]))
            port=[vars.D1D3P1, vars.D1D3P2, vars.D1D3P3, vars.D1D3P4] 
            if not utils.retry_api(verify_queuing,vars.D1, port_list=port, queue="UC3",
                                   retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]):
                success = False
            ...
            port=[vars.D3T1P1] 
            if not utils.retry_api(verify_queuing,vars.D3, port_list=port, queue="MC12",
                                                                          retry_count=3, delay=10):
                success = False
...
def test_FtOpSoRoEvpnQos325(l2_uniform_fixture):
    ...
    port=[vars.D2D1P1,vars.D2D1P2,vars.D2D1P3,vars.D2D1P4]
    if not utils.retry_api(verify_queuing,vars.D2, port_list=port, queue="MC12",
                           retry_count=3, delay=10,val_list=["10000"],tol_list=["1000"]):
        success = False
...
def test_FtOpSoRoEvpnQos331(scale_fixture):
    ...
    for dut,port,queue,dname,queue1 in zip([vars.D3,vars.D2],[[vars.D3T1P1],[vars.D2T1P1]],
        ...
        hdrMsg("verify traffic towards TGEN is queued into {} in {}".format(queue,dname))
        if utils.retry_api(verify_queuing,dut, port_list=port, queue=queue,
                           retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]) or \
           utils.retry_api(verify_queuing,dut, port_list=port, queue=queue1,
                           retry_count=3, delay=10,val_list=["5000"],tol_list=["4000"]):
...
def test_FtOpSoRoEvpnQos3217(icmp_fixture):
    ...
    for port,dname,queue in zip([[vars.D1D3P1, vars.D1D3P2, vars.D1D3P3, vars.D1D3P4],
                                ...
                                ["DUT3","DUT2"],["UC5","UC2"]):
        hdrMsg("verify traffic towards {} is queued into {} in DUT1".format(dname,queue))
        if utils.retry_api(verify_queuing,vars.D1, port_list=port, queue=queue,retry_count=3, delay=10) or \
           utils.retry_api(verify_queuing,vars.D1, port_list=port, queue=queue,retry_count=3, delay=10,
    ...
    for dut,port,queue,dname in zip([vars.D3,vars.D2],[[vars.D3T1P1],[vars.D2T1P1]],["UC1","UC6"],["DUT3","DUT2"]):
        hdrMsg("verify traffic towards TGEN is queued into {} in {}".format(queue,dname))
        if utils.retry_api(verify_queuing,dut, port_list=port, queue=queue,retry_count=3, delay=10) or \
           utils.retry_api(verify_queuing,dut, port_list=port,queue=queue,retry_count=3, delay=10,
                                                     val_list=["10000"],tol_list=["1000"]) :          
            st.log("######### traffic is queued to {} as expected ##########".format(queue))
        else:
            success = False
