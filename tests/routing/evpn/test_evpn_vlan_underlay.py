...
def test_evpn_vlan_underlay_014():
    ...
    for trigger,tc in zip(trigger_list,tc_list):
        ...
        result = utils_obj.retry_api(ip_bgp.check_bgp_session, data.dut3, nbr_list=[dut4_3_ip_list[1],dut4_3_ip_list[2]],
                           state_list=['Established'])
        if not result:
            err = "BGP neighborship failed after {}.".format(trigger)
            err_list.append(err);
            tc_result = False
            st.error("FAIL:" + err)
        ...
        result = verify_lvtep_vlanint_routes()
        if result is False :
            err = "Routes to remote svtep from leaf1 not found ."
            err_list.append(err);
            tc_result = False
            st.error("FAIL:" + err)
