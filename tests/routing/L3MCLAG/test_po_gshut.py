...
def debug_traffic_fail():
    ...
    co_utils.exec_all(dbg_exec, [[intf.show_interface_counters_all, dut] for dut in dut_list])
...
def verify_portchannel(po_name,duts,expect_states):
    ### This function checks PO state as well as member port states
    # This can be used when all  member port's state expected is same as PO state
    ver_flag = True
...
def verify_mclag_fallback(po_name,fb_cfg_states,fb_op_states,report_failure='YES',comparison_flag=True):
    ...
    result = utils.retry_parallel(po.verify_lacp_fallback, dut_list=data.mclag_peers,
                                dict_list=[dict1, dict2],retry_count=3, delay=2)
    if not result:
...
def test_po_gshut_with_fallback():
    ...
    for po_name in ['PortChannel5']:
        ### Verify D1 PO5 state & member states
        if not verify_portchannel(po_name,[dut1],['down']):
            po_fail += 1
            loop_result = False
        ### Verify D2 PO5 state
        state_dict = []
        for i in range(len([dut2])):
            state_dict += [{'portchannel': po_name, 'state': 'up'}]
        ### Verify PO state is as expected
        if not utils.retry_parallel(po.verify_portchannel_state, dict_list=state_dict, dut_list=[dut2], retry_count=3,
                                    delay=2):
            print_log("Verify PortChannel-{} State FAILED".format(po_name), "ERROR")
            po_fail += 1
            loop_result = False
    ...
    loop_result = True
    for po_name in ['PortChannel5']:
        #### Verify D1 PO5 state and member port states
        if not verify_portchannel(po_name,dut1,['down']):
            po_fail += 1
            loop_result = False
        #### Verify D2 PO5 state
    ...
    for po_name in ['PortChannel5']:
        ...
        ##### Verify D2 PO5 state
        state_dict = []
        for i in range(len([dut2,dut4])):
            state_dict += [{'portchannel': po_name, 'state': 'up'}]
        ### Verify PO state is as expected
        if not utils.retry_parallel(po.verify_portchannel_state, dict_list=state_dict, dut_list=[dut2,dut4], retry_count=3,
                                    delay=2):
            print_log("Verify PortChannel-{} State FAILED".format(po_name), "ERROR")
            po_fail += 1
            loop_result = False
    ...
    if not loop_result:
        ...
        tc1_result = False
        tc1_msg += "PO-5 states failed with fallback non-operational in standby & gshut enabled in active node:"
        debug_po_fail()
        collect_techsupport()
    else:
