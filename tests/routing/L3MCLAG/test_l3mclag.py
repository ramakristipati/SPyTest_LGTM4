...
def test_l3mclag_ceta001():
    ...
    if st.get_ui_type() in ['rest-put', 'rest-patch', 'rest-post']:
        st.report_unsupported("test_execution_skipped", "Skipping OC-Yang test case for ui_type={}".format(st.get_ui_type()))
    plat_leaf1 = basic.get_hwsku(leaf1).lower()
    plat_leaf2 = basic.get_hwsku(leaf2).lower()
    th3_plats = st.get_datastore(leaf1, "constants")['TH3_PLATFORMS']
    if plat_leaf1 in th3_plats or plat_leaf2 in th3_plats:
        st.report_unsupported("test_execution_skipped", "Skipping as this is not supported in TH3 platform. leaf1={}, leaf2={}.".format(plat_leaf1, plat_leaf2))
    ...
    if False in set(res):
        fail_msg = "ERROR: Step T4 Ping failed in unique_IP scenario."
        fail_msgs += fail_msg
        print_log(fail_msg, "MED")
        retvar = False
        more_debugs(duts=dut_list)
