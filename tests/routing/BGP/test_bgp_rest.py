...
#     """
    ...
    if not poll_wait(verify_bgp_neighbor_rst, 60, vars.D1, bgp_rst_data.dut2_ip_l[0]):
        st.report_fail("bgp_neighbor_create_delete", "v4", "Creation", "REST", "FAILED")
...
def test_ft_bgp_rst002():
    ...
    if not poll_wait(verify_bgp_neighbor_rst, 60, vars.D1, bgp_rst_data.dut2_ip6_l[0]):
        st.report_fail("bgp_neighbor_create_delete", "v6", "Creation", "REST", "FAILED")
...
def test_ft_bgp_rst004():
    ...
    for each in out:
        if not each:
            st.report_fail("bgp_neighbor_create_delete", "v6", "Creation", "REST", "FAILED")
    if not poll_wait(verify_bgp_neighbor_rst, 60, vars.D1, "Vlan{}".format(bgp_rst_data.vlan_id[0])):
        st.report_fail("bgp_neighbor_create_delete", "v6", "Creation", "REST", "FAILED")
...
def test_ft_bgp_rst005():
    ...
    for each in out:
        if not each:
            st.report_fail("bgp_neighbor_create_delete", "v6", "Creation", "REST", "FAILED")
    if not poll_wait(verify_bgp_neighbor_rst, 60, vars.D1, bgp_rst_data.portchannel_name):
        st.report_fail("bgp_neighbor_create_delete", "v6", "Creation", "REST", "FAILED")
