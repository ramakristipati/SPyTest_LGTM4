...
def prologue_epilogue(request):
    ...
    if not retry_api(verify_installed_routes,retry_count=30,delay=2):
        st.report_fail('test_execution_skipped', 'Error in module config')
    yield
    base_ecmp_deconfig()
...
def test_reboot_convergence(prologue_epilogue):
    func_result=True
    bgp_api.config_bgp(dut=data.dut1, local_as=data.dut1_as, config='yes',
                       config_type_list=['max_path_ebgp'],
                       max_path_ebgp=1, remote_as=data.tgd1_as, connect=1)
    ...
    data['table_header'] = ['Trigger']+['Iteration-{}'.format(i+1) for i in range(data.iteration_count)]
    data['table_data'] = []
    for trigger in ['coldboot','fastboot','warmboot']:
        data['table_data_1'] = []
        data.tech_support = True
        for iter in range(data.iteration_count):
            if iter+1 == 1:
                data['table_data_1'].append(trigger)
            ####################################################
            st.banner("Trigger : {} Iteration : {}".format(trigger,iter+1))
            ####################################################
            result = reboot_convergence(streams=data['underlay_ecmp'],trigger=trigger,iteration=iter+1)
            if not result:
                func_result = False
                data.tech_support = False
        data['table_data'].append(data['table_data_1'])
    table = tabulate(data['table_data'], headers=data['table_header'], tablefmt="grid")

    st.log("\n\n"+table)
    if not func_result:
        st.report_fail('test_case_failure_message','Underlay reboot convergence test failed')
    st.report_pass('test_case_passed')
...
def convergence_ecmp_underlay(dut,port_flap_list,streams,iteration):
    ...
    if not retry_api(verify_installed_routes):
        result = False
        if tech_support:debug_show();st.generate_tech_support(dut=None,name='underlay_ecmp_onfail')
        tech_support = False
...
def reboot_convergence(streams,trigger,iteration):
    traffic_rate=data.traffic_rate
    streams = [streams] if type(streams) is not list else list(streams)
    direction= '1'
    result = True;
    ...
    if not retry_api(verify_installed_routes):
        result = False;
        if tech_support:debug_show();st.generate_tech_support(dut=None,name='{}_{}'.format(trigger,iteration))
        tech_support = False
    ...
    if trigger == 'coldboot':
        st.reboot(data.dut1)
    elif trigger == 'fastboot':
        st.reboot(data.dut1,'fast')
    elif trigger =='warmboot':
        st.reboot(data.dut1,'warm')
    st.wait(60,'Wait for max routes to reinstall')
    ##########################################################
    st.log("\n\n>>>>>>>> Verify BGP sessions after Trigger {} <<<<<<\n\n".format(trigger))
    ##########################################################
    if not verify_bgp():
        result = False
        if tech_support:debug_show();st.generate_tech_support(dut=None,name='{}_{}'.format(trigger,iteration))
    ...
    ##########################################################
    st.log("\n\n>>>>>>>> Verify max routes installed after Trigger {} <<<<<<\n\n".format(trigger))
    ##########################################################
    if not retry_api(verify_installed_routes,retry_count=20,delay=5):
        result = False
        if tech_support:debug_show();st.generate_tech_support(dut=None,name='{}_{}'.format(trigger,iteration))
        tech_support = False
    ####################################################
    ...
    ####################################################
    if not retry_api(verify_traffic,tx_port=tx_tgen_port,rx_port=rx_tgen_port,field='packet_rate',tx_stream_list=tx_stream,rx_stream_list=rx_stream,mode='aggregate',
                      direction=direction,retry_count=20,tgen_obj=data.tg1,delay=6):
    ...
    if int(total_rx_count) == 0:
        st.error("Traffic Failed: RX port did not receive any pkts after {}".format(trigger))
        result =False
        if tech_support: debug_show();st.generate_tech_support(dut=None, name='{}_{}'.format(trigger,iteration))
        tech_support = False
        convergence_time='Fail'
    ...
    if convergence_time > 50.0:
        if tech_support: debug_show();st.generate_tech_support(dut=None, name='{}_{}'.format(trigger,iteration))
        tech_support = False;result=False

    data['table_data_1'].append(convergence_time)
    return result
...
def verify_bgp():
    ...
    result = retry_api(ip_bgp.check_bgp_session,data.dut1,nbr_list=nbr_list,
                       state_list=['Established']*len(nbr_list),retry_count=30,delay=5)
    if result is False:
...
def verify_installed_routes():
    v4_leaf1_count = ospf.fetch_ip_route_summary(data.dut1, key='fib_ebgp')
    v4_leaf2_count = ospf.fetch_ip_route_summary(data.dut2, key='fib_ebgp')
...
def verify_traffic(tx_port="", rx_port="", tx_ratio=1, rx_ratio=1,
    ...
    return tgapi.validate_tgen_traffic(traffic_details=traffic_details, mode="streamblock",
                                       comp_type=field, tolerance_factor=0.5)
...
def debug_show():
    st.banner(" Debug command Starts")
    def dut1():
        Intf.interface_status_show(data.dut1)
        Intf.show_interfaces_counters(data.dut1)
        st.wait(5,'wait before fetching interface counters')
        Intf.show_interfaces_counters(data.dut1)

    def dut2():
        Intf.interface_status_show(data.dut2)
        Intf.show_interfaces_counters(data.dut2)
        st.wait(5, 'wait before fetching interface counters')
        Intf.show_interfaces_counters(data.dut2)

    st.exec_all([[dut1], [dut2]])
    st.banner(" Debug command End")