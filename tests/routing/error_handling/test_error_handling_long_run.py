...
def test_ft_eh_cetasonic21955():
    ...
    st.reboot(vars.D1)
    output = bgpapi.show_bgp_ipv4_summary_vtysh(vars.D1, cli_type="vtysh", skip_tmpl=True)
    if "Error" in output:
        report_flag = "VTYSH"
    output = bgpapi.show_bgp_ipv4_summary_vtysh(vars.D1, cli_type="klish", skip_tmpl=True)
    if "Error" in output:
