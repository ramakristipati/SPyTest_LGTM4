...
def test_pim_func_006(pim_func_006_cleanup):
    ...
    ##################################################################
    st.log("\n Getting initial PRUNE_TX count on LHR1 towards FHR1 \n")
    get_packet_count(data.dut3,pkt_type='prune_tx',interface=data.d3d1_vlan_intf[0],vrf='default')
    get_packet_count(data.dut3, pkt_type='prune_tx',interface=data.d3d1_vlan_intf_vrf[0],vrf=vrf_name)

    st.log("\n Getting initial PRUNE_TX count towards FHR2 \n")
    get_packet_count(data.dut3,pkt_type='prune_tx',interface=data.d3d2_lag_intf_1,vrf='default')
    get_packet_count(data.dut3, pkt_type='prune_tx', interface=data.d3d2_lag_intf_2,vrf=vrf_name)
