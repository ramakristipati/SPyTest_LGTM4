...
class Net(object):
    ...
    def change_passwd(self, devname, username, password):
        ...
        for _ in range(0,3):
            ...
            if re.search("New password:", output):
                output = self._send_command(access, password, r"Retype new password:\s*$", delay_factor=delay_factor)
                self.logger.debug("OUTPUT: {}".format(output))
        ...
        if opts.confirm:
            ...
            if re.match(r".*y\/n\]\:?\s*$", op, re.IGNORECASE | re.DOTALL):
                if opts.expect_reboot:
                    ...
                    op = self._send_cmd_opts(devname, str(opts.confirm), expected_prompt, opts, False)
                    op_lines.append(op)
                    op = self._send_cmd_opts(devname, "", expected_prompt, opts)
                    op_lines.append(op)
    ...
    def _send_cmd_opts(self, devname, cmd, expected_prompt, opts, new_line=True):
        devname = self._check_devname(devname)
        access = self._get_dev_access(devname)
        output = self._send_command(access, cmd, expected_prompt,
        ...
        try:
            ...
            if not skip_waiting:
                if (prompt == expected_prompt or prompt == user_mode) and cmd_timetaken < 100:
                    ...
                    msg = msg + " So waiting for a static period of {} seconds.".format(opts.reboot_wait)
                    self.dut_log(devname, msg, lvl=logging.WARNING)
                    self.wait(opts.reboot_wait)
                    prompt = self._find_prompt(access)
